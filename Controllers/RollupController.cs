    ...
    public async Task<ActionResult<int>> RollupEvents([FromForm] DateTime dateToRollup)
    {
        ...
        try
        {
            ...
            using (IDocumentSession session = docstore.OpenSession())
            {
                ...
                var events = session.Query<VaccinationEvent>().Where(x => x.EventTimestamp.Year == dateToRollup.Year && x.EventTimestamp.Month == dateToRollup.Month && x.EventTimestamp.Day == dateToRollup.Day).ToList();
